...
def main(cfg):
    ...
    # model
    model = AugTrainer(cfg)
    model.to(device)
    # optmization
    optimizer = eval("torch.optim.{}".format(cfg.run.optim.optimizer))([p for p in model.parameters() if p.requires_grad], lr=cfg.run.optim.lr, weight_decay=cfg.run.optim.weight_decay)
